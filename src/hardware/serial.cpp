...
namespace mb::hw::serial
{
  ...
  {
    // TODO BMB: Simplify the "in_isr" checks to a single const variable. They are redundant.
  ...
  int SerialDriver::write( const etl::icircular_buffer<uint8_t> &buffer, const size_t length )
  {
    // TODO BMB: Need to evaluate this function. Autogenerated with Copilot.
    // TODO BMB: Simplify the "in_isr" checks to a single const variable. They are redundant.
    ...
    auto write_span = mConfig.txBuffer->write_reserve( write_size );
    buffer.read( write_span.data(), write_size );
    mConfig.txBuffer->write_commit( write_span );
  ...
  int SerialDriver::read( etl::icircular_buffer<uint8_t> &buffer, const size_t length, const size_t timeout )
  {
    ...
    size_t copy_size = available_data.size();
    {
    ...
    read_exit_critical( available_data );
  ...
  etl::span<uint8_t> SerialDriver::read_enter_critical( const size_t length, const size_t timeout )
  {
    ...
    -------------------------------------------------------------------------*/
    return mConfig.rxBuffer->read_reserve( read_size );
  }


  void SerialDriver::read_exit_critical(etl::span<uint8_t> &span )
  {
    if( span.size() != 0 )
    {
