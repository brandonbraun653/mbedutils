...
namespace mb::db
{
  ...
  NvmKVDB::NvmKVDB() : mDeviceName( "" ), mPartitionName( "" ), mSectorSize( 0 ), mNvmDBReady( ~DRIVER_INITIALIZED_KEY ), mDB( {} )
  {
  }
  ...
  DBError NvmKVDB::configure( Config &config )
  {
    ...
    ram_cfg.node_storage = config.node_storage;
    ram_cfg.transcode_buffer = config.transcode_buffer;
  ...
  int NvmKVDB::write_fdb_node( const KVNode &node, const void *data, const size_t size )
  {
    if( node.pbFields )
    {
      pb_ostream_t stream = pb_ostream_from_buffer( mTranscodeBuffer.data(),
                                                    mTranscodeBuffer.max_size() );
  ...
  int NvmKVDB::write_fdb_blob( const HashKey key, const void *data, const size_t size )
  {
    ...
    LOG_TRACE( "Write %d bytes to NVM for key %d: %s", size, key, format_array( blob.buf, blob.size ) );
  ...
  bool NvmKVDB::sync_node( const KVNode &node )
  {
    if( node.flags & KV_FLAG_PERSISTENT )
    {
      ...
      if( !fdb_kv_get_blob( &mDB, hash_to_fdb_key( node.hashKey ).c_str(),
                            fdb_blob_make( &blob, mTranscodeBuffer.data(),
                                           mTranscodeBuffer.max_size() ) ) )
      {
      ...
      LOG_TRACE( "Read %d bytes from NVM for key %d: %s", blob.saved.len, node.hashKey,
